class Checker #(
    ...
    function void check_pushes(
        ...
        for (int dst = 0; dst < DRVRS; dst++) begin
            ...
            if (item == null) begin
                ...
                $error(
                    "[%0t] [CHK] error interno: entrega encontrada pero no disponible",
                    m.t
                );
    ...
    function void check_pops(
        ...
        for (int src = 0; src < DRVRS; src++) begin
            ...
            // El DUT realmente hizo pop, por lo tanto el elemento
            // esperado avanza aunque se haya detectado mismatch.
            exp = sb.consumir_frente(src);


            // A partir de la transaccion esperada se construyen
            // las salidas que deberan aparecer posteriormente.
            sb.esperar_entrega(
                exp,
                m.t
            );
    ...
    function void final_check();
        ...
        if (restantes != 0) begin
            ...
            $error(
                "[CHK] La simulacion termino con %0d elementos esperados pendientes",
                restantes
            );
